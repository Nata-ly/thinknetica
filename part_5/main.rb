...
class TextInterface
  ...
  def choose_train(message = false, choice = false)
    ...
    Train.train_list[gets.chomp.to_i - 1] if choice == true
  end

  def choose_route(message = false, choice = false)
    puts 'Выберите маршрут' if message == true
    Route.routs_array.each_with_index { |route, index| puts "#{index + 1} - #{route}" }
    Route.routs_array[gets.chomp.to_i - 1] if choice == true
  end
  ...
  def choose_wagon(message = false, choice = false)
    ...
    Wagon.wagon_list[gets.chomp.to_i - 1] if choice == true
  end
  ...
  def choose_station(message = false, choice = false)
    ...
    Station.station_list[gets.chomp.to_i - 1] if choice == true
  end
  ...
  def work_with_trains
    loop do
      ...
      when '5'
        choose_train(true, true).move_next_station
      when '6'
        choose_train(true, true).move_past_station
      when 'stop', '0'
        break
      end
    end
